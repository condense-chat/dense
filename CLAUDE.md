...
- **Releasing.** `./scripts/release.sh` computes the next `vX.Y.Z` from the
  commit log (git-cliff), bumps `Cargo.toml`, regenerates `CHANGELOG.md`,
  commits, and tags; pushing the tag builds the matrix and publishes the GitHub
  Release. Full procedure in CONTRIBUTING.md — not in the user-facing README.
